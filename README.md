...
### 感应式入库模块 `/api/induction`

| 方法 | 路由 | 说明 |
|------|------|------|
| POST | `/api/induction/entry/scan` | 感应式入库扫码触发（感应货架自动识别） |
| POST | `/api/induction/entry/commit` | 感应式入库确认 |
| POST | `/api/induction/pick/query` | 感应式出库领料单查询 |
| POST | `/api/induction/pick/start` | 感应式出库开始（锁定条码 + 亮灯） |

### SignalR Hub `/hubs/induction`

感应式货架通过 SignalR 与 PDA 实时通信，自动推送亮灯指令，无需手动触发。
...
1. PDA 通过 SignalR Hub 建立实时连接
2. 感应货架检测到托盘/条码 → 自动推送到 Hub → PDA 接收并调用分配/查询 API
3. 入库：自动分配库位 → 通过 LightService 亮灯引导
4. 出库：查询领料单 → FIFO 锁定条码 → 亮灯引导拣货 → 完成后释放库位
...
| 文件 | 环境 | 说明 |
|------|------|------|
| `appsettings.json` | 基础 | 连接字符串、日志、版本配置（已排除 git 追踪，含敏感信息） |
| `appsettings.Production.json` | Production | 生产环境覆盖（已排除 git 追踪） |

> **安全说明**：appsettings 系列文件包含数据库密码等敏感信息，已加入 `.gitignore`，不会提交到仓库。部署时需手动配置。
